...
def calculate_collision_vector_and_jacobians(
    ...
    # Align the Z axis with the distance vector so the Jacobian has 1 row.
    target_vec = np.array([0.0, 0.0, 1.0])
    R = rotation_matrix_to_target_vector(distance_vec, target_vec)
    ...
    return (R @ distance_vec)[2], (R @ Jcoll1)[2, :], (R @ Jcoll2)[2, :]


def rotation_matrix_to_target_vector(vec, target_vec, min_norm=1e-9):
    """
    Finds a rotation matrix to convert a vector to a desired unit vector.

    Generated by ChatGPT and tested/modified manually.

    Parameters
    ----------
        vec : array-like
            The vector to align with the target vector.
        target_vec : array-like
            The target vector.
        min_norm : float
            The minimum vector norm to consider vectors aligned.

    Return
    ------
        array-like
            The rotation matrix that aligns the vector to the target.
    """
    ...
    # If the vector is already along the target, no rotation is needed.
    if np.allclose(vec, target_vec):
        return np.eye(3)
    ...
    # If the vectors are parallel, we should return the identity matrix.
    if axis_norm < min_norm:
        return np.eye(3)
    ...
    cos_angle_complement = 1.0 - cos_angle
    sin_angle = np.sin(angle)
    return np.array(
        [
            [
                cos_angle + axis_x**2 * cos_angle_complement,
                axis_x * axis_y * cos_angle_complement - axis_z * sin_angle,
                axis_x * axis_z * cos_angle_complement + axis_y * sin_angle,
            ],
            [
                axis_y * axis_x * cos_angle_complement + axis_z * sin_angle,
                cos_angle + axis_y**2 * cos_angle_complement,
                axis_y * axis_z * cos_angle_complement - axis_x * sin_angle,
            ],
            [
                axis_z * axis_x * cos_angle_complement - axis_y * sin_angle,
                axis_z * axis_y * cos_angle_complement + axis_x * sin_angle,
                cos_angle + axis_z**2 * cos_angle_complement,
            ],
        ]
    )
